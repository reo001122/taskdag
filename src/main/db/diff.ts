 ...
 *
 * 見ているのは projects / tasks / childTasks / edges の4つだけ。 タスクグラフに
 * 属さないもの(表示設定など)は、この経路を通らず db/settings.ts が直接書く
 * (CLAUDE.md §1 の明示された例外)。両者を混ぜないこと —— 差分に乗らないテーブルを
 * この経路へ載せると、書いたつもりの値が差分に現れず、静かに消える。
 */
