...
調査した限り、この層は各自が自前の仕組みで抱えています —— プレーンテキスト、リポジトリ内の
markdown、org-mode、コード内の TODO コメント。そして共通して2つのことが起きています。

- **順序が頭の中にしか無い。** 書き出したリストは順序を持たないので、「どれが今できるのか」を
  そのつど自分で思い出す必要があります。
- **中断すると復元にコストがかかる。** 半日離れただけで、どこまで進んでいて次に何をするはず
  だったかを組み立て直すことになります。

taskdag は、この構造を**そのまま描けるようにする**ものです。順序を矢印として描いておけば、
「今どれに着手できるか」は覚えておくものではなく、**画面を見れば分かるもの**になります。

## なぜ既存のツールではないのか

この形は珍しいものではなく、同じ不足を訴える声は各所で見つかります。それでも、調査した範囲の
ツールでは埋まっていませんでした。

**タスク管理ツール**(Todoist、Things 3、TickTick、Microsoft To Do、OmniFocus、Jira、Linear、
GitHub Projects)は、作業を階層かリストとしてモデル化します。依存関係は、あったとしても後付け
であり、絵にはなりません。GitHub Issues だけは例外で、登録した「これがこれをブロックしている」
という関係から状態が自動的に決まります —— **仕組みとしては taskdag と同じ**です。ただし扱うのは
チケット単位で、状態は一覧のバッジとして読むものであり、依存の全体像は見えません。

**キャンバス系ツール**(Miro、tldraw、Excalidraw、FigJam、Obsidian Canvas、Heptabase、
Workflowy、Logseq)は、ノードとエッジを描けますが、**エッジは飾り**です。矢印を引いても、
そこから何も計算されません。

片方は依存のデータを持つが絵がない。もう片方は絵を持つが何も計算しない。**taskdag はその
組み合わせです** —— 自分で編集するグラフがあり、Ready と Blocked はそのグラフから計算される。
手で管理することはありません。

調査の詳細と一次情報は `research/` にあります。
