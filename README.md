...
います。依存関係は、あったとしてもモデルの一部ではなく後付けです。

ひとつだけ例外があります。GitHub Issues では Issue 同士に「これがこれをブロックしている」
という関係を登録でき、**ブロックされている状態は手でラベルを貼らずとも自動的に付きます**。
状態を登録した関係から決める、という**仕組みだけ**は taskdag と同じです。

ただし GitHub が扱うのはチケット単位の作業で、taskdag が対象とするのはその**下の粒度**です。
そして GitHub でその状態を読むのは Issue 一覧に出るバッジであり、**依存の全体像を絵として
見ることも、その絵の上で操作することもできません**。
